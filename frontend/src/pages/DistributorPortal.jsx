...
import { fetchOrganizations, fetchDistributorMatches, federalTriMatch } from '../utils/api'
...
export default function DistributorPortal() {
  ...
  if (initLoading) return <div className="text-center py-12 text-gray-400">Loading distributors...</div>
      ...
      {matches?.distributor && (
        <div className="bg-gray-900 border border-gray-700 rounded-xl p-5">
          <div className="flex items-start justify-between">
            <div>
              <h2 className="font-bold text-white text-lg">{matches.distributor.name}</h2>
              <p className="text-sm text-gray-400 mt-1">{matches.distributor.services_description || matches.distributor.description}</p>
              <div className="flex flex-wrap gap-1.5 mt-2">
                {(matches.distributor.naics_codes || []).map(n => (
                  <span key={n} className="text-xs px-2 py-0.5 bg-purple-600/30 text-purple-300 rounded font-mono">NAICS {n}</span>
                ))}
                {matches.distributor.uei && (
                  <span className="text-xs px-2 py-0.5 bg-gray-800 text-gray-300 rounded font-mono">UEI: {matches.distributor.uei}</span>
                )}
              </div>
            </div>
            <div className="text-right text-sm text-gray-400">
              <p className="text-white font-bold">{matches.distributor.service_radius_miles} mi radius</p>
                ...
                <div key={i} className="bg-gray-900 border border-gray-700 rounded-xl p-5">
                  ...
                  {m.supplier_partners.length > 0 && (
                    <div className="border-t border-gray-800 pt-3">
                      <p className="text-xs text-gray-500 font-bold mb-2 uppercase tracking-wider">Available Supplier Partners:</p>
                      <div className="grid gap-2">
                        {m.supplier_partners.slice(0, 3).map((sp, j) => (
                          <div key={j} className="bg-gray-800 rounded-lg p-3 flex items-center justify-between">
                            <div>
                              <p className="text-sm font-bold text-white">{sp.supplier.name}</p>
                              <p className="text-xs text-gray-400">
                                {sp.distance_to_distributor} mi from you
                                {sp.pre_registered_capacity > 0 && ` — ${sp.pre_registered_capacity} items pre-registered`}
                              </p>
                              {sp.supplier.uei && (
                                <span className="text-xs text-gray-500 font-mono">UEI: {sp.supplier.uei}</span>
                              )}
                            </div>
                            <div className="text-right">
                              <p className="text-sm font-black text-amber-400">{sp.capability_match}%</p>
        ...
        <div className="space-y-5">
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-5 space-y-4">
            <p className="text-sm text-gray-400">
              Search for distribution routes by destination ZIP. Find the best supplier partners for your fleet.
            </p>
            <div>
            ...
            <button onClick={handleMatch} disabled={matchLoading || !matchZip}
              ...
              {matchLoading ? 'Searching...' : 'Find Routes & Partners'}
            </button>
          </div>
